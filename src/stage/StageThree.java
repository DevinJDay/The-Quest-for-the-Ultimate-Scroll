...
public class StageThree extends GameStage {
	...
	public StageThree(int rn, double difficulty) {
		rooms = rn;
	...
	private void initialization(int rn, double difficulty) {
		...
		int y = 10;
		coordinate[y][x] = 1;
		map.put(1, new Room3(1, x, y, im, rn, difficulty));
		...
		while (t != rn) {
			...
			if (coordinate[ny][nx] == 0) {
				coordinate[ny][nx] = ++t;
				map.put(t, new Room3(t, nx, ny, im, rn, difficulty));
				arr.add(t);
			}
			if (coordinate[ty - 1][tx] != 0 && coordinate[ty + 1][tx] != 0 && coordinate[ty][tx - 1] != 0
					&& coordinate[ty][tx + 1] != 0) {
				arr.remove(index);
			}
		}
