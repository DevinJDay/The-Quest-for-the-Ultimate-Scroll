...
public class StageTwo extends GameStage{
	...
	public StageTwo(int rn, double difficulty) {
		rooms = rn;
	...
	private void initialization(int rn, double difficulty) {
		...
		int y = 10;
		coordinate[y][x] = 1;
		map.put(1, new Room2(1, x, y, im, rn, difficulty));
		...
		while (t != rn) {
			...
			if (coordinate[ny][nx] == 0) {
				coordinate[ny][nx] = ++t;
				map.put(t, new Room2(t, nx, ny, im, rn, difficulty));
				arr.add(t);
			}
